...
from networks.MedNeXt.mednextv1.blocks import *

class MedNeXt_EffiDec3D(nn.Module):
    ...
    def __init__(self, 
        ...
        self.out_1 = OutBlock(in_channels=n_channels, n_classes=n_classes, dim=dim)
            
        # Used to fix PyTorch checkpointing bug
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)  

        if deep_supervision and mode == 'train':
            self.out_2 = OutBlock(in_channels=n_channels, n_classes=n_classes, dim=dim)
            self.out_3 = OutBlock(in_channels=n_channels, n_classes=n_classes, dim=dim)
            self.out_4 = OutBlock(in_channels=n_channels, n_classes=n_classes, dim=dim)
