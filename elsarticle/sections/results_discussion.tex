...
\subsubsection{Macro-Metric Performance Recovery ($R_{\text{rec}}$)}
Figure \ref{fig:macro_recovery} presents the macro-metric Dice recovery curves $R_{\text{rec}}(k)$ under entropy-guided block routing:
\begin{itemize}
...
\caption{Macro-metric Dice recovery curves ($R_{\text{rec}}$) under entropy-guided vs. random selective routing.}
