...
Macro recovery & Macro Recovery $R_{\text{recovery}}(k)$ & How much of the full decoder's Dice gap is recovered by boundary routing? \\
...
\subsection{Spatial Concentration and Macro-Metric Recovery}
...
\end{equation}
We record $K_{80}$, defined as the smallest fraction of spatial blocks required by a perfect selector to recover 80\% of the total positive decoder gain, establishing an empirical limit under selective prediction \cite{geifman2017selective}. A selective hybrid prediction $\hat{y}_k(v)$ executes the full decoder on selected blocks $S_k$ and the efficient baseline elsewhere:
\begin{equation}
...
\end{cases}
\end{equation}
To evaluate macro-metric impact, we compute the macro Dice recovery fraction $R_{\text{recovery}}(k)$:
\begin{equation}
R_{\text{recovery}}(k) = \frac{\text{Dice}(\hat{y}_k) - \text{Dice}(\hat{y}_e)}{\text{Dice}(\hat{y}_f) - \text{Dice}(\hat{y}_e)}
\end{equation}
which quantifies the proportion of the full decoder's macro Dice advantage recovered by executing the full decoder on a $k\%$ spatial block budget.

\subsection{Predictability, Uncertainty Signals, and TTA Mutual Information}
...
\end{equation}
where $p_c(v)$ represents the softmax probability for class $c$ at voxel $v$. 

To test whether richer multi-sample epistemic uncertainty can resolve directional predictability, we evaluate Test-Time Augmentation Mutual Information (TTA-MI) $I(v)$ over $K=8$ spatial view transformations:
\begin{equation}
I(v) = H(\bar{p}(v)) - \frac{1}{K}\sum_{k=1}^K H(p_k(v))
\end{equation}
where $p_k(v)$ is the softmax prediction under the $k$-th spatial transformation and $\bar{p}(v) = \frac{1}{K}\sum_{k=1}^K p_k(v)$ is the ensemble mean. Physically, $I(v)$ measures multi-view epistemic disagreement.

We evaluate predictability across three measures:
\begin{enumerate}
    \item \textbf{Location AUROC/AUPRC}: Discrimination of active blocks ($A(r) > 0$) vs. inactive blocks ($A(r) = 0$) \cite{hendrycks2017baseline}.
    \item \textbf{Direction AUROC/AUPRC}: Discrimination of net positive blocks ($G(r) > 0$) vs. net negative blocks ($G(r) < 0$) \cite{cmig2025quality}. We further inspect the conditional entropy density distributions $p(H \mid P(v)=1)$ and $p(H \mid N(v)=1)$ to visualize flip class overlap.
    \item \textbf{Macro-Metric Recovery ($R_{\text{recovery}}$)}: Practical Dice recovery achieved by entropy-guided block selection vs. random selection under identical spatial budgets.
\end{enumerate}
