    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        for i in range(window_data.shape[0]):
            #x_in = window_data[:,:,:,:,i] #for 1/2 batch
            x_in = window_data[i,:,:,:,:]
        ...
        #p1, p2, p3, p4 = model.decoder(f3[:,:,:,:,:f3.shape[-2]],[f2[:,:,:,:,:f2.shape[-2]],f1[:,:,:,:,:f1.shape[-2]],f0[:,:,:,:,:f0.shape[-2]]])
        ...
        #o4 = F.interpolate(o4, scale_factor=32, mode='trilinear')
        #p1, p2, p3, p4, p5, p6 = predictor.backbone(window_data, *args, **kwargs)  # batched patch segmentation
        #seg_prob_out = p1
        #seg_prob_out = predictor(window_data, *args, **kwargs)  # batched patch segmentation #p1 #+ p2 + p3 + p4
        #p1, p2 = predictor(window_data, *args, **kwargs)
        seg_prob_out = o1 + o2 + o3 + o4 #p1 + p2 + p3 + p4 + p5 + p6
        # convert seg_prob_out to tuple seg_prob_tuple, this does not allocate new memory.
        seg_prob_tuple: Tuple[torch.Tensor, ...]
        if isinstance(seg_prob_out, torch.Tensor):
            seg_prob_tuple = (seg_prob_out,)
